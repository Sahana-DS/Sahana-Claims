...
claim.attachments = attachments.map(att => ({
  name: att.file_name,
  url: `http://13.61.8.29:3762/uploads/${encodeURIComponent(att.file_path)}`,
  size: att.file_size
}));
    }
...
app.get('/api/claims/:id', async (req, res) => {
  ...
  try {
    ...
    claim.attachments = attachments.map(att => ({
      name: att.file_name,
      url: `http://13.61.8.29:3762/${att.file_path}`,
      size: att.file_size
    }));
...
app.post('/api/claims', upload.array('attachments'), async (req, res) => {
  ...
  try {
    ...
    claim.attachments = attachments.map(att => ({
      name: att.file_name,
      url: `http://13.61.8.29:3762/${path.basename(att.file_path)}`,
      size: att.file_size
    }));
...
app.listen(port, async () => {
  ...
  console.log(`Server running on http://13.61.8.29:${port}`);
});
